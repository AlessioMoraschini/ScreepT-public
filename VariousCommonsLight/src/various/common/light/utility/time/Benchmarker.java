...
public class Benchmarker {
	...
	public String getSortedReportString() {
		...
		for(int i = 0; i < elapsedList.size() - 1; i++) {
			percentSortedMap.put(elapsedList.get(i+1)/(double)totalElapsed * 100D, sortedRecords.get(i+1));
		}
		...
		for(Double key : reverseOrderKeys) {
			TimeRecord rec = percentSortedMap.get(key);
			builder.append(i + ": " + StringWorker.getStringDouble(key, 2))
				.append("% >> ")
				.append(rec.message)
				.append(" (Elapsed: " + ConversionUtils.coolTimeFromMsOrNs(rec.elapsed, false) + ")")
				.append("\n");
